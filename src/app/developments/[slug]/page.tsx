...
import { UnitType, MasterplanLot } from '@/types';
...
import { MapPin, Sparkles, Check, ArrowRight, ShieldCheck, Download, Navigation } from 'lucide-react';
import Link from 'next/link';

export default function DevelopmentDetailPage() {
  const params = useParams();
  const slug = params?.slug as string;
  const project = getDevelopmentBySlug(slug);
  ...
  const handleOpenVisitForUnit = (unit: UnitType) => {
    setSelectedUnitForVisit(project.slug);
    setIsVisitModalOpen(true);
  };

  const handleSelectLot = (lot: MasterplanLot) => {
    setSelectedUnitForVisit(project.slug);
    setIsVisitModalOpen(true);
  };
      ...
      <section id="masterplan" className="py-24 sm:py-32 bg-[#071326] text-[#F8F6F1]">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 space-y-12">
          ...
          <MasterplanViewer
            ...
            onSelectLotForVisit={handleSelectLot}
          />
        </div>
      </section>
        ...
        <div className="space-y-12">
          {project.unitTypes.map((unit) => (
            <UnitTypeCard
              key={unit.id}
              unit={unit}
              onInquire={handleOpenVisitForUnit}
            />
          ))}
        </div>
      </section>
